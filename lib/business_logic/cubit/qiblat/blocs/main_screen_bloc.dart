...
class MainBloc extends Bloc<int, MainState> {
  //final field city name and constructor
  final String cityName;
  final String zone;

  MainBloc({required this.cityName, this.zone = 'Sepang'})
      : super(MainState(weatherIcons: [])) {
    this.add(1);
  }
  ...
  @override
  Stream<MainState> mapEventToState(int event) async* {
    ...
    if (azanTimes != null && event == 1) {
      ...
      yield MainState(
          timeNow: hoursTimesFormat(azanTimes.last),
          temperature: temp,
          azansTimesList: azansTimesList,
          nextAzan: catchNextAzan(azanTimes) - 1,
          nextAzanDuration: getNextAzanDuration(
              azanTimes[catchNextAzan(azanTimes) - 1]
                  .difference(azanTimes.last)),
          nextAzanTime: azanTimes[catchNextAzan(azanTimes) - 1]
              .difference(azanTimes.last),
          weatherIcon: icon,
          weatherIcons: todaysWidgets);
    }
  }
}
